...
describe('DiaryService', () => {
  ...
  describe('createDiary', () => {
    ...
    it('use_theme이 true일 경우 theme_id와 함께 저장된다.', async () => {
      ...
      const actual = await service.create(requestUser, dto);
  ...
  describe('update', () => {
    it('일기를 수정합니다.', async () => {
      ...
      await service.update(requestUser, updateDiaryDto);
    ...
    it('존재하지 않는 ID를 수정하려고 할 경우 DiaryNotFoundException을 던집니다.', async () => {
      ...
      await expect(service.update(requestUser, updateDiaryDto)).rejects.toThrow(DiaryNotFoundException);
      expect(mockFindById).toHaveBeenCalledWith(updateDiaryDto.id);
    ...
    it('일기 수정 기간이 지났을 경우 DiaryEditExpiredException을 던집니다.', async () => {
      ...
      await expect(service.update(requestUser, updateDiaryDto)).rejects.toThrow(DiaryEditExpiredException);
      expect(mockFindById).toHaveBeenCalledWith(updateDiaryDto.id);
      expect(mockUpdate).toHaveBeenCalledTimes(0);
    });
    ...
    it('다른 사용자의 일기를 수정하려고 하면 DiaryForbiddenException이 발생해야 한다.', async () => {
      ...
      await expect(service.update(requestUser, updateDiaryDto)).rejects.toThrow(DiaryForbiddenException);
      expect(mockFindById).toHaveBeenCalledWith(updateDiaryDto.id);
      expect(mockUpdate).toHaveBeenCalledTimes(0);
    });
  });
  ...
  describe('delete',  () => {
    it('성공적으로 삭제합니다.', async () => {
      const deleteDiaryDto: DeleteDiaryDto = {
        requestUser: { id: 'uuid' },
        diaryId: 'id',
      };
      ...
      await service.delete(deleteDiaryDto);

      expect(findByIdMock).toHaveBeenCalledWith(deleteDiaryDto.diaryId);
    ...
    it('존재하지 않는 ID를 삭제하려고 하면 DiaryNotFound이 발생해야 한다.', async () => {
      ...
      await expect(service.delete(deleteDiaryDto)).rejects.toThrow(DiaryNotFoundException);
    ...
    it('다른 사용자의 일기를 삭제하려고 하면 DiaryForbiddenException이 발생해야 한다.', async () => {
      ...
      await expect(service.delete(deleteDiaryDto)).rejects.toThrow(DiaryForbiddenException);
      expect(removeMock).not.toHaveBeenCalled();
    });
  });
});
